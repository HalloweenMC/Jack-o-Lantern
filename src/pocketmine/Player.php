...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	protected function processLogin(){
		...
		parent::setHealth(($nbt["Health"] <= 0) ? 20 : $nbt["Health"]);
	...
	public function save($async = false){
		...
		if($this->level instanceof Level){
			...
			$this->namedtag["Hunger"] = $this->food;
	...
	public function setHealth($amount){
		...
		if($this->spawned === true){
			$this->getAttributeMap()->getAttribute(Attribute::HEALTH)->setValue($this->getHealth());
			$this->foodTick = 0;
			$this->sendAttributes();
		}
	}
	...
	public static function getChunkCacheFromData($chunkX, $chunkZ, $payload, $ordering = FullChunkDataPacket::ORDER_COLUMNS){
		...
		$batch->payload = zlib_encode(Binary::writeIntTag(strlen($pk->getBuffer())) . $pk->getBuffer(), ZLIB_ENCODING_DEFLATE, Server::getInstance()->networkCompressionLevel);
